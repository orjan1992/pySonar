...
class LosController(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stopped_event.wait(self.timeout) and self.wp_counter < len(self.wp_list):
            # self.lock.acquire()
            alpha = np.arctan2((self.wp_list[self.wp_counter][1] -
                                self.wp_list[self.wp_counter - 1][1]),
                               (self.wp_list[self.wp_counter][0] -
                                self.wp_list[self.wp_counter - 1][0]))
            e = -(self.pos_msg.lat - self.wp_list[self.wp_counter][0]) * np.sin(alpha) + \
                (self.pos_msg.long - self.wp_list[self.wp_counter][1]) * np.cos(alpha)
            ...
            chi = chi_r + alpha
            if first:
                if abs(chi - self.pos_msg.psi) < 4*np.pi/180.0:
                    first = False
