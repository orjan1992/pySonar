function [fig, leg_text, leg] = plot_obs_outline_on_map(file, color, fig, line, new_pos)
    if ~exist('file', 'var')
    ...
    if iscell(obs)
        for i = 1:s(2)
            ...
            leg(1) = plot([e; e(1)], [n; n(1)], line, 'Color', color);
        end
    else
        for i = 1:s(1)
            s_obs = size(obs);
...
%     plot(e, n, 'Color', color);
    leg(3) = plot(e, n, line, 'Color', color);
%     plot(e(3:4), n(3:4), '*');
%     plot(pos(2), pos(1), 'r*')
