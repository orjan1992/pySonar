...
%% Heading
heading_plot = figure();
% plot(ap_time, unwrap(wrapToPi(yaw))*180/pi, ap_time, unwrap(wrapToPi(yaw_ref))*180/pi);%, los_time, unwrap(chi)*180/pi);
plot(ap_time, wrapToPi(yaw)*180/pi);%, ap_time, wrapToPi(yaw_ref)*180/pi);%, los_time, wrapToPi(chi)*180/pi);
legend('Real', 'Reference signal', 'Los output');
xlim([datetime(date(1), date(2), date(3), 15, 30, 00), datetime(date(1), date(2), date(3), 15, 45, 30)])
xlabel('Time');
ylabel('Yaw - [Deg]');
