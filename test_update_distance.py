...
grid.oLog[200:1400, 200:1400] = np.random.random(np.shape(grid.oLog[200:1400, 200:1400]))
...
# oLog = np.random.random((11, 11))
#
# old_x, old_y = np.multiply(xy_mesh_unit, last_distance)
# new_xy = x*distance
# tmp = np.array([old_x.ravel(), old_y.ravel()]).T
# new_grid = griddata(tmp, oLog.ravel(), (new_xy,new_xy), method='linear', fill_value=5)
# # zfun_smooth_rbf = Rbf(old_xy, old_xy, oLog, function='linear', smooth=0)  # default s
# # z_dense_smooth_rbf = zfun_smooth_rbf(new_xy, new_xy)
# plt.contourf(old_xy, old_xy, oLog)
# plt.show()