...
class MtHeadData(Sensor):
    ...
    def __init__(self, byte_array, *kwargs):
        ...
        try:
            ...
            if (hex_length[0] + 6) < len(byte_array):
                raise CorruptMsgException
            ...
            (self.ad_interval, self.l_lim, self.r_lim, self.step,
             self.bearing, self.dbytes) = struct.unpack('<HHHBHH', byte_array[32:44])

            if self.adc8on:
                self.data = np.array(list(byte_array[44:(hex_length[0] + 5)]), dtype=np.uint8)
            else:
            ...
            if byte_array[hex_length[0] + 5] != 10:
